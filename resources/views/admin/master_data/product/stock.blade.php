...
<script>
	function exportData(){
		...
		window.location = "{{ url('admin/master_data/product/stock/export') }}?search=" + search + "&warehouse=" + warehouse + "&code=" + code;
   }
   
   function printData(){
		var search = $('#search').val(), page = {{ Request::get('page') ? Request::get('page') : '1' }}, warehouse = $('#warehouse').val(), code = $('#code').val();
		
		window.open("{{ url('admin/master_data/product/stock/print') }}?search=" + search + "&warehouse=" + warehouse + "&page=" + page + "&code=" + code, "_blank");
   }
</script>