						...
						<table>
							...
							<tr>
								<td style="text-align:center;">
									<h4>
										<b>
											Inventory Product Report in Qty Real
			...
			<table border="1" cellpadding="3" cellspacing="0" style="width:100%; font-size:13px;">
				...
				<tbody>
					@foreach($data as $key => $row)
						@php
							$selisih = ($row['qty'] - $row['qty_in_so']) >= 0 ? $row['qty'] - $row['qty_in_so'] : 0;
						@endphp
						<tr>
							...
							<td align="center">{{ $row['product_sqm'] }}</td>
							<td align="center">{{ round($row['qty_in_so']) }}</td>
							<td align="center">{{ $row['product_sqm'] * round($row['qty_in_so']) }}</td>
							<td align="center">{{ $row['product_sqm'] }}</td>
							<td align="center">{{ round($selisih) }}</td>
							<td align="center">{{ $row['product_sqm'] * round($selisih) }}</td>
							@if(in_array(1, session('bo_role')) || in_array(9, session('bo_role')))
							<td align="right">{{ number_format(round($selisih) * $row['price'],2,',','.') }}</td>
							@endif
						</tr>
						@php
							$total += round($selisih) * $row['price'];
							$no++;
						@endphp
					@endforeach
				</tbody>
				...
				<tfoot>
					<tr>
						<th colspan="14" align="right" style="font-size:20px !important;">Total</th>
						<th colspan="2" align="right" style="font-size:20px !important;">Rp{{ number_format($total,2,',','.') }}</th>
					</tr>
				</tfoot>
				@endif
			</table><br>
