...
<html lang="en">
	<head>
		...
		<title>Return Memo Delivery No. {{$sample->sampleDelivery->code }}</title>
		<style>
			body {
					...
					<td colspan="2">
						<table>
							<tr>
								...
								{{$sample->sampleDelivery->sample->sales->branch == '1' ? 'Surabaya' : 'Jakarta' }}, {{ date('d M Y',strtotime($sample->created_at)) }}
								</td>
							</tr>
						</table>
					</td>
						...
						<table>
							<tr>
								<td style="text-align:center;padding-top:10px;padding-bottom:10px;">
									<h2><b>MEMO RETURN BARANG (MARKETING / SALES)</b></h2>
								</td>
							</tr>
						</table>
					</td>
				...
				<tr>
					<td>
						@php
							$adatile = false;
							$adalain = false;
							foreach($sample->sampleReturnMemoDetail as $key => $pp){
						...
						Dengan Hormat,
						<br><br>
						Berikut saya buat memo return barang untuk pengiriman nomor {{$sample->sampleDelivery->code }} dan proyek nomor {{$sample->customerSample->code }}, dengan alasan <b>{{$sample->reason }}</b>, dan detail barang sebagai berikut:
						<br><br>
						<table border="1" cellpadding="5" cellspacing="0" style="width:100%; font-size:10px;">
							...
							<tbody>
								@foreach($sample->sampleReturnMemoDetail as $key => $pp)
									...
									<tr>
										...
										<td style="vertical-align:center;">
											<center>
												{{ number_format($pp->qty,2,',','.') }} <b>{{$pp->unit()}}</b>
											</center>
										</td>
									</tr>
									@php
							...
							<tbody>
								@foreach($sample->sampleReturnMemoDetail as $key => $ppku)
									@php
									...
									<tr>
										...
										<td style="vertical-align:center;">
											<center>
												{{ number_format($ppku->qty,0,',','.') }} <b>{{$ppku->unit()}}</b>
											</center>
										</td>
									</tr>
									@php
						...
						Kepada pelanggan :
						<br>
						{{$sample->sampleDelivery->receiver_name }}
						<br><br>
						<b>Alamat Pengambilan Return :
						<br>
						{{$sample->sampleDelivery->address }}
						<br><br>
