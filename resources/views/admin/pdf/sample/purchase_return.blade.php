...
<html lang="en">
	<head>
		...
		<title>Purchase Return {{ $sample->code }}</title>
		<style>
			body {
	...
	<body>
		<div class="invoice-box">
						...
						<table>
							<tr style="background-color:#ebb220;">
								<td style="text-align:center;color:white;padding-top:10px;padding-bottom:10px;">
									<h3><b>RETURN OF GOODS TO SUPPLIER</b></h3>
								</td>
							</tr>
						</table>
					</td>
					...
					<td colspan="2">
						<table width="100%">
							<tr style="line-height:0 !important;">
								...
								<td style="text-align:left; font-size:10px;">: {{ $sample->code }}</td>
								<td></td>
								<td></td>
								<td width="20%" style="font-size:10px;">Date</td>
								<td style="text-align:left; font-size:10px;">: {{ $sample->date }}</td>
							</tr>
							<tr style="line-height:0 !important;">
								<td width="20%" style="font-size:10px;">Purchase No.</td>
								<td style="text-align:left; font-size:10px;">: {{ $sample->samplePurchase->code }}</td>
								<td></td>
								<td></td>
								<td width="20%" style="font-size:10px;">Supplier</td>
								<td style="text-align:left; font-size:10px;">: {{ $sample->samplePurchase->supplier->name }}</td>
							</tr>
							<tr style="line-height:0 !important;">
								<td width="20%" style="font-size:10px;">Sales No.</td>
								<td style="text-align:left; font-size:10px;">: {{ $sample->samplePurchase->sample ? $sample->samplePurchase->sample->code : 'None' }}</td>
								<td></td>
								<td></td>
								<td width="20%" style="font-size:10px;">From Warehouse</td>
								<td style="text-align:left; font-size:10px;">: {{ $sample->warehouse->name }}</td>
							</tr>
							<tr style="line-height:0 !important;">
			...
			@php
				...
				$totaltile = 0;
				$totallain = 0;
				foreach($sample->samplePurchaseReturnProduct as $key => $pp){
			...
			<table border="1" cellpadding="5" cellspacing="0" style="width:100%; font-size:10px;">
				...
				<tr style="background:#ebb220;text-align:center;">
					...
					<th colspan="2" align="right">Rp{{ number_format($sample->getTotal(),2,',','.') }}</th>
				</tr>
			</table>
			<br>
			<table cellpadding="0" cellspacing="0">
				...
				<tr>
					...
					<td style="text-align:center;">
						...
						@else
							<br><br><br>
						@endif
						<div style="font-size:10px;font-weight:700;">{{ $sample->approve->name }}</div>
						<div style="font-size:10px;font-weight:700;">{{ $sample->approve->userRole->first()->role() }}</div>
					</td>
					@endif
				</tr>
			</table>
