...
<html lang="en">
	<head>
		...
		<title>Return Memo Delivery No. {{ $project->projectDelivery->code }}</title>
		<style>
			body {
					...
					<td colspan="2">
						<table>
							<tr>
								...
								{{ $project->projectDelivery->projectSale->sales->branch == '1' ? 'Surabaya' : 'Jakarta' }}, {{ date('d M Y',strtotime($project->created_at)) }}
								</td>
							</tr>
						</table>
					</td>
				...
				<tr>
					<td>
						@php
							$adatile = false;
							$adalain = false;
							foreach($project->projectReturnMemoDetail as $key => $pp){
						...
						Dengan Hormat,
						<br><br>
						Berikut saya buat memo return barang untuk pengiriman nomor {{ $project->projectDelivery->code }} dan proyek nomor {{ $project->project->code }}, dengan alasan <b>{{ $project->reason }}</b>, dan detail barang sebagai berikut:
						<br><br>
						<table border="1" cellpadding="5" cellspacing="0" style="width:100%; font-size:10px;">
							...
							<tbody>
								@foreach($project->projectReturnMemoDetail as $key => $ppku)
									@php
						...
						Kepada pelanggan :
						<br>
						{{ $project->projectDelivery->receiver_name }}
						<br><br>
						<b>Alamat Pengambilan Return :
						<br>
						{{ $project->projectDelivery->address }}
						<br><br>
						Alasan : {{ $project->reason }}
						<br><br>
						</b>
