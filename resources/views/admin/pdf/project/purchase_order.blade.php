...
<html lang="en">
	...
	<body>
		<div class="invoice-box">
			...
			@if($project->supplier_id != 181 && $project->letter_head == 1|| $project->letter_head == NULL)
				...
				<tr>
					<td>
						<table>
							<tr>
								<td width="55%">
									<table style="border: 1px solid {{$color}};border-collapse: collapse;">
										<tr>
											<td style="background-color:{{$color}};text-align:center;color:white;padding-top:10px;padding-bottom:10px; font-size:10pt;"><h3><b>PURCHASE ORDER</b></h3></td>
										</tr>
									</table>
								...
								<td width="40%" rowspan="6">
									<table style="border: 1px solid black;border-collapse: collapse;">
								   		@if($project->supplier_id != 181 && $project->letter_head == 1)
										<tr>
										...
										<tr>
											<td width="40%" style="font-size:10px;">Order from Buyer</td>
											<td width="5%" style="font-size:10px;">:</td>
											<td style="text-align:left; font-size:10px;"><b>PT PERWIRA SEJATI INTERNUSA
												<br>
											JALAN RAYA BUMI MASPION II VII-18 C, ROMOKALISARI,<br>
											BENOWO, SURABAYA, JAWA TIMUR, INDONESIA</b>									
											</td>
										</tr>
										@endif
										<tr>
									...
									<table style="border: 1px solid black;border-collapse: collapse;">
										<tr>
											...
											<td width="5%" style="font-size:10px;">:</td>
											<td style="text-align:left; font-size:10px;"><b>{{ $project->payment_method }} days</b></td>
										</tr>
										<tr>
								...
								<td width="55%">
									<table style="border: 1px solid black;border-collapse: collapse;">
										...
										<tr>
											...
											<td style="text-align:left; font-size:10px;">: <b>{{ $project->projectSale ? $project->projectSale->code : '-' }}</b></td>
										</tr>
										<tr>
									...
									<table style="border: 1px solid black;border-collapse: collapse;">
										<tr class="heading">
											<td colspan="2"><div style="font-size:10px;"><b>Order To Supplier :</b></div></td>
										</tr>
										<tr>
				...
				<table border="1" cellpadding="5" cellspacing="0" style="width:100%; font-size:10px;">
					<thead>
						<tr style="background:{{$color}};text-align:center;">
							<th style="color:white;" colspan="13"><center>DETAIL OF PURCHASE ORDER</center></th>
						</tr>
					</thead>
					<thead>
						<tr style="background:{{$color}};text-align:center;">
						...
						<tr style="background:{{$color}};text-align:center;">
							...
							<th style="color:white;"><center>PRICE/box</sup></center></th>
							<th style="color:white;"><center>TOTAL</center></th>
							<!--<th style="color:white;"><center>CONT</center></th>-->
						</tr>
					</thead>
					<tbody>
						@foreach($project->projectPurchaseProduct()->orderBy('id')->get() as $key => $pp)
							@php
								if($pp->unit == '2' || $pp->unit == '3'){
									...
									if($project->ppn == '1'){
										if($m2 < 1.1 && $pp->product->type->category->parent()->id !== 18){
											...
											if($m2 < 1.1 && date('Y-m',strtotime($project->created_at)) < '2022-06' && $pp->product->type->category->parent()->id == 18){
												...
												$total += round(($pp->price / $ppnpembagi) * $pp->qty * $m2, 0);
										...
										if($m2 < 1.1 && $pp->product->type->category->parent()->id !== 18){
											...
											if($m2 < 1.1 && date('Y-m',strtotime($project->created_at)) < '2022-06' && $pp->product->type->category->parent()->id == 18){
												...
												$totaltile += round($pp->price * $m2 * $pp->qty, 0);
											}
										}
							...
							<tr>
								...
								<td style="vertical-align:center;">
									<center>
										{{ $project->currency_id == 5 ? 'Rp' : $project->currency->symbol }}  {{ number_format( $project->ppn == '1' ? $pp->price / $ppnpembagi : $pp->price, 2, ',', '.') }}
									</center>
								</td>
								<td style="vertical-align:center;">
									<center>
					  					@if($m2 < 1.1 && $pp->product->type->category->parent()->id !== 18)
											   Rp. {{ number_format( $project->ppn == '1' ? ($pp->price * $project->currency_rate) / $ppnpembagi : ($pp->price * $project->currency_rate), 2, ',', '.') }}
										@else
											@if($m2 < 1.1 && date('Y-m',strtotime($project->created_at)) < '2022-06' && $pp->product->type->category->parent()->id == 18)
												Rp. {{ number_format( $project->ppn == '1' ? ($pp->price * $project->currency_rate) / $ppnpembagi : ($pp->price * $project->currency_rate), 2, ',', '.') }}
											@else
												Rp. {{ number_format( $project->ppn == '1' ? ($pp->price * $project->currency_rate * $m2) / $ppnpembagi : ($pp->price * $project->currency_rate * $m2), 2, ',', '.') }}
											@endif
										@endif
									</center>
								</td>
								<td style="vertical-align:center;">
									<center>
										@if($m2 < 1.1 && $pp->product->type->category->parent()->id !== 18)
											{{ $project->currency_id == 5 ? 'Rp' : $project->currency->symbol }}  {{ number_format($project->ppn == '1' ? ($pp->price / $ppnpembagi) * $pp->qty : $pp->price * $pp->qty, 0, ',', '.') }}
										@else
											@if($m2 < 1.1 && date('Y-m',strtotime($project->created_at)) < '2022-06' && $pp->product->type->category->parent()->id == 18)
												{{ $project->currency_id == 5 ? 'Rp' : $project->currency->symbol }}  {{ number_format($project->ppn == '1' ? ($pp->price / $ppnpembagi) * $pp->qty : $pp->price * $pp->qty, 0, ',', '.') }}
											@else
												{{ $project->currency_id == 5 ? 'Rp' : $project->currency->symbol }}  {{ number_format($project->ppn == '1' ? round(($pp->price / $ppnpembagi) * $pp->qty * $m2, 0) : round($pp->price * $pp->qty * $m2, 0), 0, ',', '.') }}
											@endif
										@endif
									</center>
								</td>
						...
						<tr>
							...
							<td colspan="3" style="text-align:right;">{{ $project->currency->symbol.' '.number_format($totaltile, 2, ',', '.') }}</td>
						</tr>
						<tr>
							<td colspan="11" style="text-align:right;">Tax</td>
							<td colspan="3" style="text-align:right;">{{ $project->ppn == '1' ? $project->currency->symbol.' '.number_format($totaltile * $persenppn, 0, ',', '.') : $project->currency->symbol.' '.number_format(0, 2, ',', '.') }}</td>
						</tr>
						<tr>
							<td colspan="11" style="text-align:right;">Grandtotal</td>
							<td colspan="3" style="text-align:right;">{{ $project->ppn == '1' ? $project->currency->symbol.' '.number_format( $totaltile + ($totaltile * $persenppn), 0, ',', '.') : $project->currency->symbol.' '.number_format($totaltile, 2, ',', '.') }}</td>
						</tr>
					</tbody>
				</table>
				@php
				...
				<table border="1" cellpadding="5" cellspacing="0" style="width:100%; font-size:10px;">
					<thead>
						...
						<tr style="background:{{$color}};text-align:center;">
							...
							<th style="color:white;"><center>SHADE</center></th>
							<th style="color:white;"><center>FINISH</center></th>
							<th style="color:white;"><center>TOTAL{{$adacustom == true ? '' : '(pcs)'}}</center></th>
							<th style="color:white;"><center>PRICE{{$adacustom == true ? '' : '/PCS'}}</center></th>
							<th style="color:white;" colspan="2"><center>TOTAL</center></th>
							<!--<th style="color:white;"><center>CONT</center></th>-->
						</tr>
					</thead>
					<tbody>
						@foreach($project->projectPurchaseProduct()->orderBy('id')->get() as $key => $ppku)
							@php
							...
							<tr>
								...
								<td style="vertical-align:center;">
									<center>
										<img src="{{ $ppku->product->type->image() }}" style="max-width:70px; border:1px solid #ddd; border-radius:4px; padding: 5px;" class="img-fluid img-thumbnail">
									</center>
								</td>
								<td style="vertical-align:center;">
									<center>
								...
								<td style="vertical-align:center;">
									<center>
										{{ number_format($ppku->qty,2,',','.') }} {{$adacustom == true ? $pp->unit() : ''}}
									</center>
								</td>
								<td style="vertical-align:center;">
									<center>
									{{ $project->currency_id == 5 ? 'Rp' : $project->currency->symbol }} {{ number_format($project->ppn == '1' ? round(($ppku->price / $ppnpembagi)) : $ppku->price, 2, ',', '.') }}
									</center>
								</td>
								<td style="vertical-align:center;" colspan="2">
									<center>
									{{ $project->currency_id == 5 ? 'Rp' : $project->currency->symbol }} {{ number_format($project->ppn == '1' ? round(($ppku->price / $ppnpembagi)) * $ppku->qty : $ppku->price * $ppku->qty, 2, ',', '.') }}
									</center>
								</td>
								<!--<td style="vertical-align:center;">-->
								<!--	<center>-->
						...
						<tr>
							...
							<td colspan="3" style="text-align:right;">{{ $project->ppn == '1' ? $project->currency->symbol.' '.number_format($totallain * $persenppn, 2, ',', '.') : 0 }}</td>
						</tr>
						<tr>
							<td colspan="10" style="text-align:right;">Grandtotal</td>
							<td colspan="3" style="text-align:right;">{{ $project->ppn == '1' ? $project->currency->symbol.' '.number_format($totallain + ($totallain * $persenppn), 0, ',', '.') : $project->currency->symbol.' '.number_format($totallain, 2, ',', '.') }}</td>
						</tr>
					</tbody>
				</table>
				@php
			...
			@if(count($project->projectPurchaseSplit) > 0)
				<table border="1" cellpadding="5" cellspacing="0" style="width:100%; font-size:10px;">
					<thead>
						<tr style="background:{{$color}};text-align:center;">
							<th style="color:white;" colspan="8"><center>DETAIL OF PRODUCT FROM ANOTHER PURCHASE ORDER</center></th>
						</tr>
					</thead>
					@if($adatile == true)
					<thead>
						...
						<tr style="background:{{$color}};text-align:center;">
							<th style="color:white;"><center>NO</center></th>
							<th style="color:white;"><center>PO. No</center></th>
							<th style="color:white;"><center>PRODUCT</center></th>
							<th style="color:white;"><center>TOTAL (box)</center></th>
							<th style="color:white;"><center>TOTAL (sqm)</center></th>
							<th style="color:white;"><center>PRICE/M<sup>2</sup></center></th>
					...
					<tbody>
						@foreach($project->projectPurchaseSplit as $key => $psp)
							@if($psp->unit == '2' || $psp->unit == '3')
								...
								<tr>
									...
									<td style="vertical-align:center;">
										<center>
											{{ $psp->projectPurchaseReference->code }}
										</center>
									</td>
									<td style="vertical-align:center;">
										<center>
									...
									<td style="vertical-align:center;">
										<center>
											{{ number_format($psp->qty,2,',','.') }}
										</center>
									</td>
									<td style="vertical-align:center;">
										<center>
											{{ number_format($psp->qty * $m2,2,',','.') }}
										</center>
									</td>
									<td style="vertical-align:center;">
										<center>
											{{ $project->currency_id == 5 ? 'Rp' : $project->currency->symbol }}  {{ number_format( $project->ppn == '1' ? $psp->price / $ppnpembagi : $psp->price, 2, ',', '.') }}
										</center>
									</td>
									<td style="vertical-align:center;">
										<center>
						...
						@foreach($project->projectPurchaseSplit as $key => $psp)
							@if($psp->unit == '1' || $psp->unit == '4')
								<tr>
									...
									<td style="vertical-align:center;">
										<center>
											{{ number_format($psp->qty,2,',','.') }} {{$adacustom == true ? $pp->unit() : ''}}
										</center>
									</td>
									<td style="vertical-align:center;">
										<center>
									...
									<td style="vertical-align:center;">
										<center>
											{{ $project->currency_id == 5 ? 'Rp' : $project->currency->symbol }}  {{ number_format($project->ppn == '1' ? ($psp->price / $ppnpembagi) * $psp->qty : $psp->price * $psp->qty, 0, ',', '.') }}
										</center>
									</td>
								</tr>
							@endif
			...
			<table style="border: 1px solid black;border-collapse: collapse;">
					...
					<tr>
						...
						<td style="text-align:left;">
							<div style="font-size:10px;">
								{{ $project->ppn == '1' ? number_format(round(($totaltile + $totallain) + (($totaltile + $totallain) * $persenppn)),0,',','.') : number_format(round($totaltile + $totallain),2,',','.') }}
							</div>
						</td>
					</tr>
				@endif
				...
				<tr>
					...
					<td style="text-align:left;">
						<div style="font-size:10px;">
							{{ $project->ppn == '1' ? App\Helper\SMB::say(round(($totaltile + $totallain) + (($totaltile + $totallain) * $persenppn))) : App\Helper\SMB::say(round($totaltile + $totallain)) }}
						</div>
					</td>
				</tr>
			</table><br><br>
				...
				<tr>
					...
					<td style="text-align:center;">
						...
						@if(isset($project->checked->sign))
							<div><img src="{{ (Storage::url($project->checked->sign)) }}" height="65px"></div>
						@else
							<br><br><br>
						@endif
						<div style="font-size:10px;font-weight:700;">{{ $project->checked->name }}</div>
						<div style="font-size:10px;font-weight:700;">{{ $project->checked->userRole->first()->role() }}</div>
					</td>
					@endif
					...
					<td style="text-align:center;">
						...
						@if(isset($project->approved->sign))
							<div><img src="{{ (Storage::url($project->approved->sign)) }}" height="65px"></div>
						@else
							<br><br><br>
						@endif
				...
				<table cellpadding="0" cellspacing="0">
					<tr>
						<td>
							<table>
								<tr>
									<td style="background-color:{{$color}};text-align:center;color:white;padding-top:10px;padding-bottom:10px;" width="55%">
									...
									<td width="55%">
										...
										<table style="border: 1px solid black;border-collapse: collapse;">
											<tr>
												...
												<td style="text-align:left; font-size:10px;">: <b>{{ $project->customer ? $project->customer->name : '-' }}</b></td>
											</tr>
											<tr>
												<td width="30%" style="font-size:10px;">Sales Person</td>
												<td style="text-align:left; font-size:10px;">: <b>{{ $project->sales ? $project->sales->name : '-' }}</b></td>
				...
				<table border="1" cellpadding="5" cellspacing="0" style="width:100%; font-size:10px;">
					...
					<tbody>
						@foreach($project->projectPurchaseProduct()->orderBy('id')->get() as $key => $pp)
							@php
								if($pp->unit == '2' || $pp->unit == '3'){
									...
									if($project->ppn == '1'){
										if($m2 < 1.1 && $pp->product->type->category->parent()->id !== 18){
											$total += (($pp->price * $project->currency_rate) / $ppnpembagi) * $pp->qty;
											$totaltile += (($pp->price * $project->currency_rate) / $ppnpembagi) * $pp->qty;
										}else{
											if($m2 < 1.1 && date('Y-m',strtotime($project->created_at)) < '2022-06' && $pp->product->type->category->parent()->id == 18){
												$total += (($pp->price * $project->currency_rate) / $ppnpembagi) * $pp->qty;
												$totaltile += (($pp->price * $project->currency_rate) / $ppnpembagi) * $pp->qty;
											}else{
												$total += (($pp->price * $project->currency_rate) / $ppnpembagi) * $pp->qty * $m2;
												$totaltile += (($pp->price * $project->currency_rate) / $ppnpembagi) * $m2 * $pp->qty;
											}
										}
										...
										if($m2 < 1.1 && $pp->product->type->category->parent()->id !== 18){
											...
											$totaltile += ($pp->price * $project->currency_rate) * $pp->qty;
										}else{
											if($m2 < 1.1 && date('Y-m',strtotime($project->created_at)) < '2022-06' && $pp->product->type->category->parent()->id == 18){
												$total += ($pp->price * $project->currency_rate) * $pp->qty;
												$totaltile += ($pp->price * $project->currency_rate) * $pp->qty;
											}else{
												$total += ($pp->price * $project->currency_rate) * $pp->qty * $m2;
												$totaltile += ($pp->price * $project->currency_rate) * $m2 * $pp->qty;
											}
							...
							<tr>
								...
								<td style="vertical-align:center;">
									<center>
										...
										@if($project->ppn == '1')
											@if($m2 < 1.1 && $pp->product->type->category->parent()->id !== 18)
												{{ number_format((($pp->price * $project->currency_rate) / $ppnpembagi) * $pp->qty,0,',','.') }}
											@else
												@if($m2 < 1.1 && date('Y-m',strtotime($project->created_at)) < '2022-06' && $pp->product->type->category->parent()->id == 18)
													{{ number_format((($pp->price * $project->currency_rate) / $ppnpembagi) * $pp->qty,0,',','.') }}
												@else
													{{ number_format((($pp->price * $project->currency_rate) / $ppnpembagi) * $pp->qty * $m2,0,',','.') }}
												@endif
											@endif
										@else
											@if($m2 < 1.1 && $pp->product->type->category->parent()->id !== 18)
												{{ number_format(($pp->price * $project->currency_rate) * $pp->qty,0,',','.') }}
											@else
												@if($m2 < 1.1 && date('Y-m',strtotime($project->created_at)) < '2022-06' && $pp->product->type->category->parent()->id == 18)
													{{ number_format(($pp->price * $project->currency_rate) * $pp->qty,0,',','.') }}
												@else
													{{ number_format(($pp->price * $project->currency_rate) * $pp->qty * $m2,0,',','.') }}
												@endif
											@endif
										@endif
									</center>
				    	...
				    	<tr>
							...
							<td style="text-align:center;">{{ $totalqtytile }}</td>
							<td style="text-align:right;">Total</td>
							<td colspan="3" style="text-align:right;">{{ 'Rp '.number_format($totaltile, 0, ',', '.') }}</td>
						</tr>
						<tr>
							<td colspan="10" style="text-align:right;">Tax</td>
							<td colspan="3" style="text-align:right;">{{ $project->ppn == '1' ? 'Rp '.number_format($totaltile * $persenppn, 0, ',', '.') : 'Rp '.number_format(0, 0, ',', '.') }}</td>
						</tr>
						<tr>
							<td colspan="10" style="text-align:right;">Grandtotal</td>
							<td colspan="3" style="text-align:right;">{{ $project->ppn == '1' ? 'Rp '.number_format( $totaltile + ($totaltile * $persenppn), 0, ',', '.') : 'Rp '.number_format($totaltile, 0, ',', '.') }}</td>
						</tr>
					</tbody>
				</table>
				@php
				...
				<table border="1" cellpadding="5" cellspacing="0" style="width:100%; font-size:10px;">
					<thead>
						...
						<tr style="background:{{$color}};text-align:center;">
							...
							<th style="color:white;"><center>TOTAL{{$adacustom == true ? '' : '(pcs)'}}</center></th>
							<th style="color:white;"><center>PRICE{{$adacustom == true ? '' : '/PCS'}}</center></th>
							<th style="color:white;"><center>RATE</center></th>
							<th style="color:white;" colspan="2"><center>TOTAL</center></th>
					...
					<tbody>
						@foreach($project->projectPurchaseProduct()->orderBy('id')->get() as $key => $pp)
							@php
								if($pp->unit == '1' || $pp->unit == '4'){
									if($project->ppn == '1'){
										$total += (($pp->price * $project->currency_rate) / $ppnpembagi) * $pp->qty;
										$totallain += (($pp->price * $project->currency_rate) / $ppnpembagi) * $pp->qty;
										$qtytot += $pp->qty;
									}else{
										$total += ($pp->price * $project->currency_rate) * $pp->qty;
										$totallain += ($pp->price * $project->currency_rate) * $pp->qty;
										$qtytot += $pp->qty;
									}
									$totalqtylain += $pp->qty;
							@endphp
								...
								<td style="vertical-align:center;">
									<center>
										{{ number_format($pp->qty,2,',','.') }} {{$adacustom == true ? $pp->unit() : ''}}
									</center>
								</td>
								<td style="vertical-align:center;">
									<center>
								...
								<td style="vertical-align:center;">
									<center>
										Rp {{ number_format($project->currency_rate,0,',','.') }}
									</center>
								</td>
								<td style="vertical-align:center;" colspan="2">
									<center>
									Rp {{ number_format($project->ppn == '1' ? (($pp->price * $project->currency_rate) / $ppnpembagi) * $pp->qty : ($pp->price * $project->currency_rate) * $pp->qty, 2, ',', '.') }}
									</center>
								</td>
						...
						<tr>
							<td colspan="10" style="text-align:right;">Tax</td>
							<td colspan="3" style="text-align:right;">{{ $project->ppn == '1' ? 'Rp '.number_format($totallain * $persenppn, 2, ',', '.') : 0 }}</td>
						</tr>
						<tr>
							<td colspan="10" style="text-align:right;">Grandtotal</td>
							<td colspan="3" style="text-align:right;">{{ $project->ppn == '1' ? 'Rp '.number_format($totallain + ($totallain * $persenppn), 2, ',', '.') : 'Rp '.number_format($totallain, 2, ',', '.') }}</td>
						</tr>
					</tbody>
				</table>
				@php
				...
				<table cellpadding="0" cellspacing="0">
					<tr>
						<td style="text-align:center;">
							<div style="font-size:10px;">PO released by</div>
							@if(isset($project->user->sign))
								<div><img src="{{ (Storage::url($project->user->sign)) }}" height="65px"></div>
							@else
								<br><br><br>
							@endif
							<div style="font-size:10px;font-weight:700;">{{ $project->user->name }}</div>
