						...
						<table>
							...
							<tr>
								<td style="padding-right:10px;padding-top:100px !important;">
									...
									<div style="font-size:9px; font-weight:500;">Jalan Raya Perancis Blok E-6 Jati
										Mulya, Dadap, Tangerang</div>
			...
			<table cellpadding="0" cellspacing="0">
				<tr>
					<td colspan="2" style="vertical-align: middle;padding-top:15px;padding-bottom:45px;">
						<center>
							<img src="{{ url('website/letterheadpsi_big.jpg') }}" width="100%"
										...
										foreach($project->projectSale->projectSaleProduct->where('product_id',$pp->product_id) as $psp){
											if($psp->unit == '2' || $psp->unit == '3'){
												...
												if($m2 < 1.1 && $pp->product->type->category->parent()->id !== 18){
													...
													if($m2 < 1.1 && date('Y-m',strtotime($project->projectSale->project->created_at)) < '2022-06' && $pp->product->type->category->parent()->id == 18){
														echo number_format($psp->best_price * $pp->qty,0,',','.');
														$total += $psp->best_price * $pp->qty;
													}else{
				...
				<tr>
					...
					<td style="text-align:center;">
						...
						@if(isset($project->approve->sign))
							<div><img src="{{ url(Storage::url($project->approve->sign)) }}" height="65px"></div>
						@else
							<br><br><br>
						@endif
						<div style="font-size:10px;font-weight:700;">{{ isset($project->approve->name) ? $project->approve->name : '' }}</div>
						<div style="font-size:10px;font-weight:700;">{{ isset($project->approve->name) ? $project->approve->userRole->first()->role() : '' }}</div>
					</td>
					<td style="text-align:center;">
