...
<head>
    ...
    <title>Sample Order {{ $sample->customerSample->code }}</title>
    <style>
        body {
...
<body>
    <div class="invoice-box">
        ...
        <table>
                ...
                <td colspan="2">
                    <table width="100%">
                        <tr style="line-height:0 !important;">
                            ...
                            <td style="text-align:left; font-size:12px;">: {{ $sample->customerSample->customer->name }}</td>
                            <td></td>
                            <td></td>
                            <td width="20%" style="font-size:12px;">SAMPLE NUMBER</td>
                            <td style="text-align:left; font-size:12px;">: {{ $sample->customerSample->code }}</td>
                        </tr>
                        <tr style="line-height:0 !important;">
                            <td width="20%" style="font-size:12px;">STATUS</td>
                            <td style="text-align:left; font-size:12px;">: {!! $sample->status() !!}</td>
                            <td></td>
                            <td></td>
                            <td width="20%" style="font-size:12px;">DATE</td>
                            <td style="text-align:left; font-size:12px;">: {{ date('d F Y',
                                strtotime($sample->sent_date)).' to '.date('d F Y', strtotime($sample->return_date)) }}
                            </td>
                        </tr>
                        <tr style="line-height:0 !important;">
                            <td width="20%" style="font-size:12px;">
                                {{ $sample->status == '2' ? 'DATE RETURNED' : '' }}
                            </td>
                            <td style="text-align:left; font-size:12px;">
                                {{ $sample->status == '2' ? ': '.$sample->returned_at : '' }}
                            </td>
                            <td></td>
                            <td></td>
            ...
            <tr>
                <td colspan="2">
                    <!-- <p style="font-size:10px;">Dear mr. {{ $sample->customerSample->customer->name }}<br>
        ...
        <table border="1" cellpadding="5" cellspacing="0" style="width:100%; font-size:10px;">
            ...
            <tbody>
                @foreach($sample->sampleProduct as $key => $psp)
                <tr>
        ...
        <table cellpadding="0" cellspacing="0">
            <tr>
                <td style="text-align:center;" width="20%">
                    ...
                    @if(isset($sample->customerSample->user->sign))
                    <div><img src="{{ url(Storage::url($sample->customerSample->user->sign)) }}" height="75px"></div>
                    @else
                    <br><br><br><br><br>
                    @endif
                    <div style="font-size:10px;">{{ $sample->customerSample->user->name }}</div>
                </td>
                @if(isset($sample->approved_1->name))
                <td style="text-align:center;" width="20%">
                    <div style="font-size:10px;">Validated By</div>
                    @if(isset($sample->approved_1->sign))
                    <div><img src="{{ url(Storage::url($sample->approved_1->sign)) }}" height="75px"></div>
                    @else
                    <br><br><br><br><br>
                    @endif
                    <div style="font-size:10px;">{{ $sample->approved_1->name }}</div>
                </td>
                @endif
                @if(isset($sample->approved_2->name))
                <td style="text-align:center;" width="20%">
                    <div style="font-size:10px;">Approved By</div>
                    @if(isset($sample->approved_2->sign))
                    <div><img src="{{ url(Storage::url($sample->approved_2->sign)) }}" height="75px"></div>
                    @else
                    <br><br><br><br><br>
                    @endif
                    <div style="font-size:10px;">{{ $sample->approved_2->name }}</div>
                </td>
                @endif
                <td style="text-align:center;" width="20%">
