			...
			<table border="1" cellpadding="5" cellspacing="0" style="width:100%; font-size:10px;">
				...
				<thead>
					...
					<tr style="background:#ebb220;text-align:center;">
						...
						<th style="color:white;"><center>PRICE (BEFORE TAX)</center></th>
						<th style="color:white;"><center>TOTAL</center></th>
					</tr>
				</thead>
				<tbody>
					@if($project->projectDelivery)
					@foreach($project->project->projectDelivery()->get() as $pd)
					@foreach($pd->projectDeliveryProduct as $key => $pp)
						@php
							if($pp->unit == '2' || $pp->unit == '3'){
								
								foreach($project->projectSale->projectSaleProduct->where('product_id',$pp->product_id) as $psp){
									...
									if($m2 < 1.1){
										...
										$total += ($psp->best_price * $m2 * $countbox) / $project->projectSale->currency_rate;
										$price = $psp->best_price * $m2 / $project->projectSale->currency_rate;
										$totaltile += ($psp->best_price * $m2 * $countbox) / $project->projectSale->currency_rate;
									}
						...
						<tr>
							...
							<td style="vertical-align:center;">
								<center>
									@php 
										echo number_format($price * $pp->qty, 0, ',', '.');
									@endphp
								</center>
							</td>
						</tr>
					...
					@endforeach
					@else
					@foreach($project->project->projectDelivery as $key => $pd)
						@foreach ($pd->projectDeliveryProduct as $key => $pp) 
						@php
							if($pp->unit == '2' || $pp->unit == '3'){
					...
					<tr>
						...
						<th>Subtotal</th>
						<th>{{ number_format($totaltile, 0, ',', '.') }}</th>
					</tr>
				</tbody>
				@php
				}
				...
				<thead>
					...
					<tr style="background:#ebb220;text-align:center;">
						...
						<th style="color:white;"><center>CATEGORY</center></th>
						<th style="color:white;"><center>COLOR</center></th>
						<th style="color:white;"><center>QTY (PCS)</center></th>
						<th style="color:white;"><center>PRICE (BEFORE TAX)</center></th>
						<th style="color:white;"><center>TOTAL</center></th>
					</tr>
				</thead>
				<tbody>
					...
					@foreach($pd->projectDeliveryProduct as $key => $pp)
						@php
							if($pp->unit == '1' || $pp->unit == '4'){
								$total += str_replace(',','.',str_replace('.','',$pp->salePrice())) * $pp->qty;
								$totallain += str_replace(',','.',str_replace('.','',$pp->salePrice())) * $pp->qty;
								$totalqtylain += $pp->qty;
						@endphp
						<tr>
							<td style="vertical-align:center;">
							...
							<td style="vertical-align:center;">
								<center>
									{{ number_format(str_replace(',','.',str_replace('.','',$pp->salePrice())) * $pp->qty, 0, ',', '.') }}
								</center>
							</td>
						</tr>
						@php
					...
					<tr>
						<td colspan="7" style="text-align:right;">Total Qty</td>
						<td style="text-align:center;">{{ $totalqtylain }}</td>
						<th>Subtotal</th>
						<th>{{ number_format($totallain, 0, ',', '.') }}</th>
					</tr>
				</tbody>
				@php
				}
					...
					<td width="45%">
						<table cellpadding="2" cellspacing="0" border="1">
							...
							<tr>
								...
								<td>
									<h6>IDR {{ number_format(($project->project->discount ? ($total - $project->project->discount) : $total), 0, ',', '.') }}</h6>
								</td>
							</tr>
							<tr>
								<td>
								...
								<td>
									<h6>IDR {{ $project->projectSale->project->ppn == '1' ? number_format($persenppn * ($total - $project->project->discount), 0, ',', '.') : number_format(0, 0, ',', '.') }}</h6>
								</td>
							</tr>
							@if ($project->projectDelivery->break_tolerance)
							<tr>
								<td>
									<h6>TOLERANSI PECAH ({{$project->projectDelivery->break_tolerance}} %)</h6>
								</td>
								<td>
									<h6> IDR {{number_format($project->projectDelivery->getTotal()['totalBreakTolerance'], 0, ',', '.')  }}</h6>
								</td>
							</tr>
							@endif
							@php
								$grandtotal = 0;
								$breakTolerance = $project->projectDelivery->break_tolerance ? $project->projectDelivery->getTotal()['totalBreakTolerance']: 0;

								if($project->projectSale->project->ppn == '1'){
									$grandtotal += ($total - $project->project->discount) + ($persenppn * ($total - $project->project->discount)) - $breakTolerance;
								}else{
									$grandtotal += ($total - $project->project->discount) - $breakTolerance;
								}
							...
							<tr>
								...
								<td>
									<h6>IDR {{ $grandtotal - $totalpaid < 0 ? '0' : number_format($grandtotal - $totalpaid - $totalreturn, 0, ',', '.') }}</h6>
								</td>
							</tr>
							<tr>
								<td>
								...
								<td>
									<h6>{{ $grandtotal - $totalpaid > 0 ? App\Helper\SMB::say(round($grandtotal - $totalpaid - $totalreturn)) : 'Zero' }} Rupiahs</h6>
								</td>
							</tr>
						</table>
					</td>
		...
		<div class="separate-box">
			<table id="table-kwitansi" style="vertical-align: top;padding: 15px;font-size:13px;background-image: url({{ url('website/bg_kwitansi.png') }});" width="100%" height="auto">
				...
				<tr>
					...
					<td width="1%">:</td>
					<td>{{ App\Helper\SMB::say($grandtotal - $totalpaid - $totalreturn) }}</td>
				</tr>
				<tr>
					<td>For payment</td>
					<td width="1%">:</td>
				...
				<tr>
					<td>Total</td>
					<td width="1%">:</td>
					<td><div style="border:1p solid black;padding:10px;">Rp {{ number_format($grandtotal - $totalpaid - $totalreturn,0,',','.') }}</div></td>
				</tr>
				<tr>
					<td></td>
					<td width="1%"></td>
						...
						<table>
							<tr>
								...
								<td style="text-align:center;">
									...
									Surabaya, {{ date('d F Y', strtotime($project->projectDelivery ? $project->projectDelivery->delivery_date : $project->project->projectDelivery->first()->delivery_date)) }}
									...
									@elseif($project->projectSale->sales->branch == '2')
										...
										Jakarta, {{ date('d F Y', strtotime($project->projectDelivery ? $project->projectDelivery->delivery_date : $project->project->projectDelivery->first()->delivery_date)) }}
										<br><br><br><br><br><br><br><br>
										(...............................)
