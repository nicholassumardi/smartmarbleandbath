					...
					<td colspan="2">
						<table>
							...
							<tr>
								...
								<td style="border-left: 3px solid {{ $color }}; text-align:right;padding-top:100px !important;">
			...
			@endphp
				...
				<tbody>
					...
					@foreach($detail as $key => $pp)
							...
							if($pp->unit == '2' || $pp->unit == '3'){
								...
								if($m2 < 1.1 && $pp->product->type->category->parent()->id !== 18){
									...
									if($m2 < 1.1 && date('Y-m',strtotime($project->project->created_at)) < '2022-06' && $pp->product->type->category->parent()->id == 18){
										...
										$pp->best_price == 0 ? $total += ($pp->recommended_price == 0 ? $pp->price : $pp->recommended_price) * $m2 * $countbox : $total += $pp->best_price * $m2 * $countbox;
										$pp->best_price == 0 ? $totaltile += ($pp->recommended_price == 0 ? $pp->price : $pp->recommended_price) * $m2 * $countbox : $totaltile += $pp->best_price * $m2 * $countbox;
									}
						...
						@endphp
						<tr>
							<td style="vertical-align:center; font-size:18pt;">
								<center>
									{{ $no }}
								</center>
							</td>
									...
									@php
										if($pp->price > 0){
										    if($pp->best_price > 0 && $pp->best_price > $pp->price || $pp->recommended_price > 0 &&  $pp->recommended_price > $pp->price)
											{
												...
												echo '<span style="color:black">'.number_format($pp->price, 0, ',', '.').'</span><img src="website/strike_through.png" style="float: left; margin: -9px 0 7px 0" height="0.005px" width="40px" /><br>';
											}else{
										...
										if($pp->recommended_price > 0 && $pp->price !== $pp->recommended_price){
											if($pp->best_price > 0){
												echo '<span style="color:black">'.number_format($pp->recommended_price, 0, ',', '.').'</span><img src="website/strike_through.png" style="float: left; margin: -9px 0 7px 0" height="0.005px" width="40px" /><br>';
											}else{
														...
														if($pqp2->product_id == $pp->product_id){
															if($pqp2->best_price == 0){
																if($pqp2->recommended_price !== $pp->recommended_price && !in_array($pqp2->recommended_price,$tempprice)){
																	echo '<span style="color:black">'.number_format($pqp2->recommended_price, 0, ',', '.').'</span><img src="website/strike_through.png" style="float: left; margin: -9px 0 7px 0" height="0.005px" width="40px" /><br>';
																}
																
																$tempprice[] = $pqp2->recommended_price;
															}else{
																if($pqp2->best_price !== $pp->best_price && !in_array($pqp2->best_price,$tempprice)){
																	echo '<span style="color:black">'.number_format($pqp2->best_price, 0, ',', '.').'</span><img src="website/strike_through.png" style="float: left; margin: -9px 0 7px 0" height="0.005px" width="40px" /><br>';
																}
									...
									@php
										if($m2 < 1.1 && $pp->product->type->category->parent()->id !== 18){
											...
											if($m2 < 1.1 && date('Y-m',strtotime($project->project->created_at)) < '2022-06' && $pp->product->type->category->parent()->id == 18){
												...
												echo $pp->best_price == 0 ? number_format(($pp->recommended_price == 0 ? $pp->price * $m2 : $pp->recommended_price * $m2), 0, ',', '.') : number_format($pp->best_price * $m2, 0, ',', '.');
											}
										}
									...
									@php
										if($m2 < 1.1 && $pp->product->type->category->parent()->id !== 18){
											...
											if($m2 < 1.1 && date('Y-m',strtotime($project->project->created_at)) < '2022-06' && $pp->product->type->category->parent()->id == 18){
												...
												echo $pp->best_price == 0 ? number_format(($pp->recommended_price == 0 ? round(($pp->price * $m2), 0) * $countbox : round(($pp->recommended_price * $m2),0) * $countbox), 0, ',', '.') : number_format(round(($pp->best_price * $m2), 0) * $countbox, 0, ',', '.');
											}
										}
									@endphp
								</center>
				...
				<tbody>
														...
														if($pqp2->product_id == $pp->product_id){
															if($pqp2->best_price == 0){
																if($pqp2->price !== $pp->price && !in_array($pqp2->price,$tempprice)){
																	echo '<span style="color:black">'.number_format($pqp2->price, 0, ',', '.').'</span><img src="website/strike_through.png" style="float: left; margin: -9px 0 7px 0" height="0.005px" width="40px" /><br>';
																}
					...
					@endforeach
					<tr>
						<th colspan="colspan="{{$project->display_brand == 1 ? 10 : 11}}"" style="text-align:right;">Total Qty</th>
						<th>{{ $totalqtylain }}</th>
						<th colspan="2">Subtotal</th>
					...
					<td width="45%">
						<table cellpadding="2" cellspacing="0" border="1">
							...
							<tr>
								...
								<td>
									<h6>IDR {{ number_format($project->project->cutting_cost, 0, ',', '.') }}</h6>
								</td>
							</tr>
							<tr>
								<td>
								...
								<td>
									<h6>IDR {{ number_format($project->project->misc_cost, 0, ',', '.') }}</h6>
								</td>
							</tr>
							<tr>
								<td>
								...
								<td>
									<h6>IDR {{ number_format($project->project->delivery_cost, 0, ',', '.') }}</h6>
								</td>
							</tr>
							<tr>
								<td>
									<h6>DISKON TAMBAHAN</h6>
								</td>
								<td>
							...
							<tr>
								...
								<td>
									<h6>IDR {{ number_format($total + ($project->project->delivery_cost + $project->project->misc_cost + $project->project->cutting_cost) - $project->project->discount, 0, ',', '.') }}</h6>
								</td>
							</tr>
							<tr>
								<td>
