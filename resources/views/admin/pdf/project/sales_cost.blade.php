...
<html lang="en">
	<head>
		...
		<title>Sales Service Order {{ $project->code }}</title>
		<style>
			body {
				...
				<tbody>
					...
					<tr>
						<td style="vertical-align:center;">
							<center>
								{{ $project->misc_note ? $project->misc_note : 'Other' }}
							</center>
						</td>
						<td>
							{{ $project->currency->code }} {{ number_format(($project->delivery_cost + $project->cutting_cost + $project->misc_cost) / $project->currency_rate,$koma,',','.') }}
						</td>
					</tr>
					<tr>
						<td><center>Subtotal</center></td>
						<td>{{ $project->currency->code }} {{ number_format(($project->delivery_cost + $project->cutting_cost + $project->misc_cost) / $project->currency_rate, $koma, ',', '.') }}</td>
					</tr>
					<tr>
						<td><center>Tax</center></td>
						<td>{{ $project->currency->code }} {{ $project->ppn_cost == '1' ? number_format((($project->delivery_cost + $project->cutting_cost + $project->misc_cost) * $persenppn) / $project->currency_rate, $koma, ',', '.') : 0 }}</td>
					</tr>
					<tr>
						<td><center>Total</center></td>
						<td>{{ $project->currency->code }} {{ $project->ppn_cost == '1' ? number_format(((($project->delivery_cost + $project->cutting_cost + $project->misc_cost) * $persenppn) + ($project->delivery_cost + $project->cutting_cost + $project->misc_cost)) / $project->currency_rate, $koma, ',', '.') : number_format(($project->delivery_cost + $project->cutting_cost + $project->misc_cost) / $project->currency_rate, $koma, ',', '.') }}</td>
					</tr>
				</tbody>
			</table>
			<br>
						...
						<table cellpadding="2" cellspacing="0" border="1" style="background-color:#51b6bc;font-size:14px;color:white;">
							<tr>
								...
								<td style="text-align:center;">
									<b>{{ $project->currency->code }} {{ $project->ppn_cost == '1' ? number_format((($project->delivery_cost + $project->cutting_cost + $project->misc_cost) + ($persenppn * ($project->delivery_cost + $project->cutting_cost + $project->misc_cost))) / $project->currency_rate, $koma, ',', '.') : number_format(($project->delivery_cost + $project->cutting_cost + $project->misc_cost) / $project->currency_rate, $koma, ',', '.') }}</b>
								</td>
							</tr>
							<tr>
								<td colspan="2">
							...
							<tr>
								<td colspan="2">
									<div>
										Nominal in words : {{ $project->ppn_cost == '1' ? App\Helper\SMB::say((($project->delivery_cost + $project->cutting_cost + $project->misc_cost) + ($persenppn * ($project->delivery_cost + $project->cutting_cost + $project->misc_cost))) / $project->currency_rate) : App\Helper\SMB::say(($project->delivery_cost + $project->cutting_cost + $project->misc_cost) / $project->currency_rate) }}
									</div>
								</td>
							</tr>
						</table>
