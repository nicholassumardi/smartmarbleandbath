...
<div class="content-wrapper">
	...
	<div class="content">
		...
		<div class="row">
					...
					<div class="card-header header-elements-inline">
						...
						<div class="header-elements">
							<div class="list-icons">
								<a class="list-icons-item" data-action="reload" onclick="reset()" data-popup="tooltip"
									title="Reset Filter"></a>
								<a href="javascript:void(0);" class="list-icons-item"
									onclick="fullScreenGas($('#detailSo'));"><i class="icon-screen-full"></i></a>
											...
											<div class="col-md-3">
												<div class="form-group">
													...
													<select name="branch" id="branch" class="form-control"
														...
														<option value="1" {{ $branch=='1' ? 'selected' : '' }}>PTA
														</option>
														<option value="2" {{ $branch=='2' ? 'selected' : '' }}>SMB
														</option>
														<option value="3" {{ $branch=='3' ? 'selected' : '' }}>MKJ
														</option>
														<option value="4" {{ $branch=='4' ? 'selected' : '' }}>PSI
														</option>
													</select>
												</div>
											</div>
									...
									<div class="col-sm-12">
										...
										<div class="row justify-content-center">
											<div class="col-sm-6">
												<div class="card" style="background-color:#3399ff !important;">
													<div class="card-body"
														style="background-color:#3399ff !important;color:white;">
														<div class="d-flex">
															<h3 class="font-weight-semibold mb-0" id="nominaltotal">
															</h3>
															<span
																class="badge bg-teal-800 badge-pill align-self-center ml-auto"
																id="percenttotal"></span>
														</div>
														<div>
															Sales Order (SO) <br>
															Pending sales Rp {{
															number_format($projectsalebefore,0,',','.') }}<br>
											...
											<div class="col-sm-6">
												<div class="card" style="background-color:#009933 !important;">
													<div class="card-body"
														style="background-color:#009933 !important;color:white;">
														<div class="d-flex">
												...
												<div class="row justify-content-center">
													...
													<div class="col-md-4">
														<a class="btn btn-warning" data-toggle="collapse"
															...
															onclick="fullScreenGas($('#detailSo'));">
															Show Full Screen
														</a>
													</div>
												</div>
										...
										<div class="row collapse mt-3" id="collapse-link-collapsed">
											<div class="col-sm-12" id="detailSo">
												...
												<h5 class="card-title">{{ date('F Y', strtotime($filter)) }} Sales
													Report Lists</h5>
												<div class="card">
													<div class="card-body">
														<ul
															class="nav nav-tabs nav-tabs-solid bg-slate border-0 nav-justified rounded">
															<li class="nav-item"><a href="#bordered-justified-tab3"
																	class="nav-link" data-toggle="tab">Pending</a></li>
															<li class="nav-item"><a href="#bordered-justified-tab1"
																	class="nav-link active" data-toggle="tab">SO</a>
															</li>
															<li class="nav-item"><a href="#bordered-justified-tab2"
																	class="nav-link" data-toggle="tab">Done</a></li>
															<li class="nav-item"><a href="#bordered-justified-tab4"
																	class="nav-link" data-toggle="tab">Return</a></li>
														</ul>
																...
																<div class="table-responsive">
																	<table id="datatable_serverside"
																		...
																		<tbody>
																			...
																			if($adadata == true){
																			if($val->is_closed &&
																			substr($val->date_closed,0,7) <= $filter){
																				}else{ $totalbefore +=$val->total_sisa;
																				}
																				@endphp
																				<tr
																					class="{{ $val->is_closed && substr($val->date_closed,0,7) <= $filter ? 'bg-danger' : '' }}">
																					<td>{{ $nomor }}</td>
																					<td>{{ $val->project->ppn == '1' ?
																						str_replace('PJ','PJP',$val->project->code)
																						:
																						str_replace('PJ','PJN',$val->project->code)
																						}}</td>
																					<td>{{ $val->code }}</td>
																					<td>{{ date('d M
																						Y',strtotime($val->created_at))
																						}}</td>
																					<td>{{ $val->project->user->name }}
																					</td>
																					<td>{{ $val->sales->name }}</td>
																					<td>{{ $val->project->customer->name
																						}}</td>
																					<td>{{ $val->project->name }}</td>
																					<td class="text-right">
																						Rp@php
																						echo
																						number_format($val->total_sisa,0,',','.');
																						@endphp
																					</td>
																				</tr>
																				@php
																	...
																	echo $rowpd->projectSale->code.'
																	'.number_format($rowpd->subtotal_product +
																	$rowpd->subtotal_service,0,',','.').'<br>';
																	$totaltrial += $rowpd->subtotal_product +
																	$rowpd->subtotal_service;
																	}
																	echo 'TOTAL :
																	'.number_format($totaltrial,0,',','.'); */
																	@endphp
																</div>
															</div>

															<div class="tab-pane fade show active"
																id="bordered-justified-tab1">
																<div class="table-responsive">
																	<table id="datatable_serverside"
																		class="table table-bordered table-striped w-100">
																		<thead class="bg-dark">
																		...
																		<tbody>
																			...
																			if($adadata == true){
																			$total +=
																			str_replace(',','.',str_replace('.','',$val->getTotalRawPlusService()));
																			@endphp
																			<tr>
																				<td>{{ $nomor }}</td>
																				<td>{{ $val->project->ppn == '1' ?
																...
																<div class="table-responsive">
																	<table id="datatable_serverside"
																		...
																		<tbody>
																			...
																			if($val->projectSale->sales->branch ==
																			$branch){
																			$adadata = true;
																			}

																			if($adadata == true){
																			$totalpaid += $val->subtotal_product +
																			$val->subtotal_service;
																			...
																			<tr>
																				...
																				<td>{{ date('d M
																					Y',strtotime($val->received_date))
																					}}</td>
																				<td>{{ $val->user->name }}</td>
																				<td>{{ $val->projectSale->sales->name }}
																				</td>
																				<td>{{ $val->project->customer->name }}
																				</td>
																				<td>{{ $val->project->name }}</td>
																				<td class="text-right">
																					Rp{{
																					number_format($val->subtotal_product
																					+ $val->subtotal_service,0,',','.')
																					}}
																				</td>
																			</tr>
																			@php
																...
																<div class="table-responsive">
																	<table id="datatable_serverside"
																		...
																		<tbody>
																			...
																			if(date('Y-m-d',strtotime($val->projectSale->created_at))
																			< '2022-04-01' ){ $ppnpembagi=1.1; }else{
																				$ppnpembagi=1.11; } $totalreturn=0;
																		...
																		<tfoot>
																			<tr class="bg-teal">
																				...
																				<th colspan="2" class="text-right">Rp{{
																					number_format($totalallreturn,0,',','.')
																					}}</th>
																			</tr>
																		</tfoot>
																	</table>
								...
								<center class="d-block">
									<div class="row justify-content-center">
										<div class="col-md-3 text-left">
											<div class="form-group">
												<label>Year</label>
												<select name="done_year" id="done_year" class="form-control"
													onchange="submitDoneSalesTarget()">
										...
										<div class="col-md-3 text-left">
											<div class="form-group">
												<label>Branch</label>
												<select name="done_branch" id="done_branch" class="form-control"
													onchange="submitDoneSalesTarget()">
									...
									<div class="row text-center">
										...
										<div class="col-4 p-3" style="background-color:#cccc00;">
											...
											<a href="javascript:void(0);"
												style="color:white;position:absolute;top:10px;right:10px;z-index:999;"
												class="badge badge-success badge-pill" data-popup="tooltip"
												title="Show Details" onclick="showDetailActualDoneSales();"><i
													class="icon-typewriter"></i></a>
										</div>

										<div class="col-4 p-3" style="background-color:#dd99ff;">
											<p id="done_status"><i
													class="icon-stats-decline2 icon-3x d-inline-block text-success"></i>
											</p>
			...
			<div class="col-xl-6" id="widgetOwner">
				<div class="card">
					<div class="card-header header-elements-inline">
						...
						<div class="header-elements">
							<div class="list-icons">
								<a class="list-icons-item" data-action="reload" onclick="getOwnerDashboard()"
					...
					<div class="card-body py-0" style="max-height: 400px;min-height:400px;overflow:auto;">
						<div class="row">
							<div class="col-sm-4 col-md-4">
								<div class="card card-body"
									style="background-color:#0395d7 !important;color:white !important;">
									<div class="media">
									...
									<div class="text-center mt-2">
										&nbsp; <a href="javascript:void(0);" style="color: white !important;"
											onclick="showOnGoingOrder();">Details</a>
									</div>
								</div>
							</div>
							<div class="col-sm-4 col-md-4">
								<div class="card card-body"
									style="background-color:#ff1ac6 !important;color:white !important;">
									<div class="media">
										<div class="media-body">
									...
									<div class="text-center mt-2">
										&nbsp; <a href="javascript:void(0);" style="color: white !important;"
											onclick="showSalesMonth();">Details</a>
									</div>
								</div>
							</div>
							<div class="col-sm-4 col-md-4">
								<div class="card card-body"
									style="background-color:#009933 !important;color:white !important;">
									<div class="media">
										<div class="media-body">
									...
									<div class="text-center mt-2">
										&nbsp; <a href="javascript:void(0);" style="color: white !important;"
											onclick="showPurchaseOrder();">Details</a>
									</div>
								</div>
							</div>
							<div class="col-sm-4 col-md-4">
								<div class="card card-body"
									style="background-color:#99994d !important;color:white !important;">
									<div class="media">
										<div class="media-body">
							...
							<div class="col-sm-4 col-md-4">
								<div class="card card-body" style="background-color:white !important;">
									...
									<div class="progress mb-2" style="height: 0.125rem;">
										<div class="progress-bar bg-success-400" style="width: 0%"
											id="percentPaymentProgress">
										</div>
									</div>
			...
			<div class="col-xl-6 col-md-12">
				<div class="card">
					<div class="card-header header-elements-inline">
						<h3 class="card-title">{{ date('F Y', strtotime($filter)) }} Cash Flow (Monthly)</h3>
						<div class="header-elements">
							<div class="list-icons">
								<a class="list-icons-item" data-action="reload" onclick="reset()" data-popup="tooltip"
									title="Reset Filter"></a>
					...
					<div class="card-body py-0" style="max-height: 400px;min-height:400px;overflow:auto;">
						<div class="row">
							<div class="col-md-12">
								<form method="GET" id="form_filter_cashflow">
								@csrf
								<center class="d-block">
									<div class="row justify-content-center">
										<div class="col-md-3">
											<div class="form-group">
												<label>Branch</label>
												<select name="branch_cf" id="branch_cf" class="form-control"
													onchange="getDataCashFlow()">
													<option value="1" {{ $branch_cashflow == '1' ? 'selected' : '' }}>PTA
													</option>
													<option value="2" {{ $branch_cashflow == '2' ? 'selected' : '' }}>SMB
													</option>
												</select>
											</div>
										</div>
										<div class="col-md-3">
											<div class="form-group">
												<label>Month Year</label>
												<input type="month" name="filter_cf" id="filter_cf" class="form-control"
													value="{{ $month ? $month : date('Y-m') }}"  onchange="getDataCashFlow()">
											</div>
										</div>
									</div>
								</center>
								...
								<div class="row justify-content-center">
									<div class="col-sm-12">
										<div class="row justify-content-center">
											<div class="col-sm-6">
												<div class="card" style="background: linear-gradient(315deg, #f39f86 0%, #f9d976 74%) !important;"
													id="infodatacashflow">
													<div class="card-body"
														style="background: linear-gradient(315deg, #f39f86 0%, #f9d976 74%) !important; color:black;">
														<div class="d-flex">
															<h3 class="font-weight-semibold mb-0" id="nominaltotal">
															</h3>
															<span
														...
														<div class="media-body">
															<h5><b>Cash Flow</b></h5>
															Debit <span id="debitcashflow">{{number_format($total_debit,0,',','.') }}</span><br>
															Credit <span id="creditcashflow">{{number_format($total_credit,0,',','.') }}</span><br>
															Balance Rp <span id="balancecf">{{number_format($balance,0,',','.') }}</span><br>
														</div>
											...
											<div class="col-sm-12">
												<div class="row justify-content-center">
													<a class="btn btn-warning" data-toggle="collapse"
														href="javascript:void(0);" onclick="showDetailCashFlow()">
														Show Detail
													</a>
												</div>
											</div>
			...
			in_array('4',session('bo_role')))
			<div class="col-xl-6" id="widgetFinance">
				<div class="card">
					<div class="card-header header-elements-inline">
						<h3 class="card-title">Cash & Bank In & Out</h3>
						<div class="header-elements">
							<div class="list-icons">
								<a class="list-icons-item" data-action="reload" onclick="getCashBankInOut()"
									data-popup="tooltip" title="Refresh Data"></a>
								<a class="list-icons-item" data-action="collapse"></a>
							</div>
						</div>
									...
									<center class="d-block">
										<div class="row justify-content-center">
											<div class="col-md-3">
												<div class="form-group">
													<label>Mode In</label>
													<select name="finance_mode_in" id="finance_mode_in"
														class="form-control" onchange="getCashBankInOut()">
											...
											<div class="col-md-3">
												<div class="form-group">
													...
													<select name="finance_mode_out" id="finance_mode_out"
														class="form-control" onchange="getCashBankInOut()">
														<option value="">All</option>
											...
											<div class="col-md-3">
												<div class="form-group">
													<label>Branch</label>
													<select name="finance_branch" id="finance_branch"
														class="form-control" onchange="getCashBankInOut()">
														<option value="">All</option>
														<option value="1">PTA</option>
														<option value="2">SMB</option>
											...
											<div class="col-md-3">
												<div class="form-group">
													...
													<input type="month" name="finance_from_month"
														id="finance_from_month" class="form-control"
														onchange="getCashBankInOut()" value="{{ date('Y-m') }}">
												</div>
											</div>
											<div class="col-md-3">
												<div class="form-group">
													<label>To Month</label>
													<input type="month" name="finance_to_month" id="finance_to_month"
														class="form-control" onchange="getCashBankInOut()"
														value="{{ date('Y-m') }}">
												</div>
											</div>
								...
								<div class="row justify-content-center mb-2">
									<div class="col-md-3">
										<a class="btn btn-secondary" href="javascript:void(0);"
											onclick="showDetailCashBank();">Show Details</a>
									</div>
								</div>
							</div>
							<!-- <div class="col-md-12">
			...
			<div class="col-xl-6">
				<div class="card">
					<div class="card-header header-elements-inline">
						<h3 class="card-title">{{ $year }} Sales Report {{ $branch == '1' ? 'PTA' : 'SMB' }} (Yearly)
						</h3>
						<div class="header-elements">
							<div class="list-icons">
								<a class="list-icons-item" data-action="reload" onclick="reset()" data-popup="tooltip"
			...
			<div class="modal-content">
				...
				<div class="modal-footer bg-light">
					<div class="mr-auto" style="font-size:25px !important;">
						Helper <i class="icon-point-right mr-2 icon-2x"></i> Red rows mean that the SO were closed by
						Sales Person.
					</div>
					<button type="button" class="btn bg-secondary closeFullScreen" data-dismiss="modal"><i
							class="icon-switch2"></i> Close</button>
				</div>
			</div>
		</div>
	</div>
	...
	{{-- @if (in_array(1, session('bo_role')))
	<div class="modal fade" id="modal_cash_flow" data-backdrop="static" role="dialog">
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header bg-light">
					<h5 class="modal-title" id="exampleModalLabel">Information</h5>
				...
				<div class="modal-body" id="informationCashFlow">
				@php
					$noweek = 1;
					$balancedebitweek = 0;
					$balancecreditweek = 0;
															...
															@if(!in_array($rowar['date'],$row))
																@if($noweek == 1 && $rowar['date'] < $month.'-01')
																	<tr class="bg-danger" data-type="{{ $rowar['type'] }}" data-id="{{ $rowar['id'] }}" data-description="{{ $rowar['description'] }}" data-nominal="{{ number_format($rowar['total'],2,',','.') }}" data-group="1">
																		...
																		<td class="text-center">{{ date('d/m/y',strtotime($rowar['date'])) }}</td>
																		<td class="text-right"{{ number_format($rowar['total'],2,',','.') }}> </td>
																...
																@foreach($datacredit as $key => $rowap)
																	@if(in_array($rowap['date'],$row))
																		<tr data-type="{{ $rowap['type'] }}" data-id="{{ $rowap['idcf'] ? $rowap['idcf'] : $rowap['id'] }}" data-description="{{ $rowap['description'] }}" data-week="{{ $noweek }}" data-group="2" class="{{ $rowap['fixedcost'] == '1' ? 'bg-success' : '' }}" data-nominal="{{ number_format($rowap['total'],2,',','.') }}">
																			...
																			<td class="text-center">{{ date('d/m/y',strtotime($rowap['date'])) }}</td>
																			<td class="text-right">{{ number_format($rowap['total'],2,',','.') }}</td>
	...
	<script>
		...
		function getDataCashFlow(){
			loadingOpen('.content');
			$('#form_filter_cashflow').submit();
		}

		function showDetailCashFlow(){
			$('#modal_cash_flow').modal('toggle');
		}
