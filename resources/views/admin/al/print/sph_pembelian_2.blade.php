				...
				<tr>
					<td colspan="2">
						<table>
							...
							<tr>
								<td style="text-align:center;">
									<h4><b>LAPORAN SPH PEMBELIAN V.2 (INTERNAL)</b></h4>
					...
					@foreach($parent as $rowparent)
						...
						<tr>
							<td style="vertical-align:center;" colspan="7">
								<b>{{ $noparent.'. '.$rowparent['name'] }}</b>
							</td>
						</tr>
						
						@foreach($data->alSphProduct()->whereHas('alProduct', function($query) use ($rowparent){ 										
							$query->where('al_category_id',$rowparent['id']);
						})->orderBy('id')->get() as $key => $ps)
							@php
							...
							<tr>
								...
								<td style="vertical-align:center;text-align:left !important;">
									{{ $key + 1 }}. {{ $ps->alProduct->name }}
								</td>
						...
						@foreach(AlCategory::where('parent_id',$rowparent['id'])->get() as $row)
							<tr>
								<td style="vertical-align:center;text-align:left !important;padding-left:50px;" colspan="7">
									{{ $rowno.'. '.$row->name }}
								</td>
							</tr>
							@foreach($data->alSphProduct()->whereHas('alProduct', function($query) use ($row){ 										
								$query->where('al_category_id',$row->id);
							})->orderBy('id')->get() as $key => $ps)
								@php
									$hpp = $ps->buy_price * $ps->qty;
									$totalhpp += $hpp;
