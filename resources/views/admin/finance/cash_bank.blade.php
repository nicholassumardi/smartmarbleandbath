...
<script>
				...
				<tr class="text-center">
				   ...
				   <td class="align-middle">
						<div class="form-check" style="padding-left: 0rem;">
							<label class="form-check-label">
							<input type="checkbox" class="form-check-input-styled-primary" value="` + nominal_detail.val() + `" name="debit_checkbox[]">
						</div>
				   </td>
				   <td class="align-middle">
					  <button type="button" id="delete_data_content_debit" class="btn bg-danger btn-sm"><i class="icon-trash"></i></button>   
   ...
   function reset() {
	...
	function create() {
		...
		let counterConfig = {};
		let dynamic_condition1 = '';
		let dynamic_condition2 = '';
		let dynamic_condition3 = '';

		 @foreach (DB::table('company_entities')->get() as $company)
			counterConfig['{{ $company->id }}'] = {
				'1': { counter: 'debet{{$company->name}}', value : 0},
				'2': { counter: 'kredit{{$company->name}}', value : 0}
			};
   		 @endforeach
		...
		$('input[name^="branch_detail"]').each(function () {
			...
			if (counterConfig[value] && counterConfig[value][mode]) {
				var counters = counterConfig[value][mode];
				counters.value++;
			}
		...
		$.each(counterConfig, function (companyId, counters) {
            dynamic_condition3 += `(${counters['1'].value}` + ` > 0 && ${counters['2'].value}` + ` > 0`;
            $.each(counterConfig, function (compIdLoopSecond, counterSecond) {
                if((companyId !== compIdLoopSecond) && (companyId === Object.keys(counterConfig).length.toString() && compIdLoopSecond === (Object.keys(counterConfig).length - 1).toString())){
					dynamic_condition1 += `(${counters['1'].value}` + ` == 1 && ${counterSecond['2'].value}` + ` == 1)`;
					dynamic_condition3 += ` && ${counterSecond['1'].value}` + ` == 0 && ${counterSecond['2'].value}` + ` == 0)`;
		...
		$.each(counterConfig, function (companyId, counters) {
			if (companyId === Object.keys(counterConfig).length.toString()) {
				dynamic_condition2 += `(${counters['1'].value}` + ` !== ${counters['2'].value})`;
			} else {
				dynamic_condition2 += `(${counters['1'].value}` + ` !== ${counters['2'].value}) || `;
			}
		});

		const resultDyanmiCon1 = eval(dynamic_condition1);
		const resultDyanmiCon2 = eval(dynamic_condition2);
		const resultDyanmiCon3 = eval(dynamic_condition3);
					...
					$('#data_content_debit').append(`
						<tr class="text-center">
						   ...
						   <td class="align-middle">
								<div class="form-check" style="padding-left: 0rem;">
									<label class="form-check-label">
									<input type="checkbox" class="form-check-input-styled-primary" value="`+ val.nominal +`" name="debit_checkbox[]" ` + (val.checked ? val.checked : ((counter == 1 ? `checked` : ``))) + `>
								</div>
						   </td>
						   <td class="align-middle">
							  <button type="button" id="delete_data_content_debit" class="btn bg-danger btn-sm"><i class="icon-trash"></i></button>   
   ...
   function update(id) {
	   var debit = 0, credit = 0;
		...
		if(jumDebet > 1 && jumKredit > 1){
			if(resultDyanmiCon2){
				multibranch = true;
				console.log('true kah manis')
			}

			if(resultDyanmiCon3) {
				multibranch = false;
				console.log('false kah manis');
				console.log(dynamic_condition3);
			}
		}
		...
		$('input[name^="credit_checkbox"]').each(function(){
			if($(this).is(":checked")){
				fd.append("credit_checkbox[]", $(this).val());
			}else{
				fd.append("credit_checkbox[]", '0');
			}
		});

		$('input[name^="debit_checkbox"]').each(function(){
			if($(this).is(":checked")){
				fd.append("debit_checkbox[]", $(this).val());
			}else{
				fd.append("debit_checkbox[]", '0');
			}
		});
