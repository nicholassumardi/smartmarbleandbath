...
use App\Models\AlSupplierCurrency;
use App\Http\Controllers\Controller;
use Illuminate\Support\Facades\Validator;

class AlSupplierController extends Controller {
    ...
    public function index()
    {
        $data = [
            ...
            'content'  => 'admin.al.master_data.supplier'
        ];
    ...
    public function datatable(Request $request) 
    {
        ...
        $total_data = AlSupplier::count();
        
        $query_data = AlSupplier::where(function($query) use ($search, $request) {
                if($search) {
        ...
        $total_filtered = AlSupplier::where(function($query) use ($search, $request) {
                if($search) {
                    $query->where(function($query) use ($search) {
                        $query->where('code', 'like', "%$search%")
                            ->orWhere('name', 'like', "%$search%")
    ...
    public function create(Request $request)
    {
        ...
        if($validation->fails()) {
            ...
            $query = AlSupplier::create([
                'country_id'      => $request->country_id,
                'code'            => AlSupplier::generateCode(),
                'name'            => $request->name,
                'email'           => $request->email,
                'phone'           => $request->phone,
            ...
            if($query) {
                if($request->currency_id) {
                    foreach($request->currency_id as $c) {
                        AlSupplierCurrency::create([
                            'al_supplier_id' => $query->id,
                            'currency_id' => $c
                        ]);
                    }
                }
    ...
    {
        $data     = AlSupplier::find($request->id);
        $currency_id = [];

        foreach($data->alSupplierCurrency as $c) {
    ...
    public function update(Request $request, $id)
    {
        ...
        if($validation->fails()) {
            ...
            $query = AlSupplier::where('id', $id)->update([
                'country_id'      => $request->country_id,
                'name'            => $request->name,
                'email'           => $request->email,
                'phone'           => $request->phone,
            ...
            if($query) {
                AlSupplierCurrency::where('al_supplier_id', $id)->delete();
                if($request->currency_id) {
                    foreach($request->currency_id as $c) {
                        AlSupplierCurrency::create([
                            'supplier_id' => $id,
